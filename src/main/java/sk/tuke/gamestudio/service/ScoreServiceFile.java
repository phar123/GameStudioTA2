...
public class ScoreServiceFile implements ScoreService {
    ...
    public List<Score> getBestScores(String game) {
        scores=load();
        return scores.stream().filter(s->s.getGame().equals(game)) //Nemusi byt zatvorka
                .sorted((s1,s2) -> -Integer.compare(s1.getPoints(), s2.getPoints())) //Musi byt v zatvorke lebo viac param
                .limit(5)
